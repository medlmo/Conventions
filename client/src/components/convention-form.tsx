...
export function ConventionForm({ open, onOpenChange, convention }: ConventionFormProps) {
  ...
  // Invalidates the list (any URL params) and the stats in one call
  const invalidateConventions = () =>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-4xl sm:max-w-5xl md:max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        {/* Tracking sections — only visible in edit mode */}
        {convention && (
          <div className="mt-6 space-y-6 border-t border-gray-200 pt-6">
            <FinancialTracking convention={convention} />
            <AdministrativeTracking convention={convention} />
          </div>
        )}
      </DialogContent>
    </Dialog>
