...
import { File, Plus, Download, Search, Eye, Edit, Trash2, LogOut, Users, Settings, BarChart3, ChevronRight, ChevronLeft } from "lucide-react";
...
export default function ConventionsPage() {
  ...
  // Chart stats — now managed by TanStack Query so invalidation works on add/edit/delete
  const { data: sectorStats = [] } = useQuery<any[]>({
    queryKey: ["/api/conventions/stats/by-sector"],
  });
  const { data: statusStats = [] } = useQuery<any[]>({
    queryKey: ["/api/conventions/stats/by-status"],
  });
  const { data: sectorCostStats = [] } = useQuery<any[]>({
    queryKey: ["/api/conventions/stats/by-sector-cost"],
  });
  const { data: domainStats = [] } = useQuery<any[]>({
    queryKey: ["/api/conventions/stats/by-domain"],
  });
  const { data: provinceStats = [] } = useQuery<any[]>({
    queryKey: ["/api/conventions/stats/by-province"],
  });
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        predicate: (query) =>
          ...
          (query.queryKey[0] as string).startsWith("/api/conventions"),
      });
      toast({
  ...
  const totalPages = Math.max(1, Math.ceil(filteredConventions.length / PAGE_SIZE));
  const paginatedConventions = filteredConventions.slice(
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-reverse space-x-2">
                <span className="text-sm text-gray-600">
                  عرض {Math.min((currentPage - 1) * PAGE_SIZE + 1, filteredConventions.length)} –{" "}
                  {Math.min(currentPage * PAGE_SIZE, filteredConventions.length)} من أصل{" "}
                  {filteredConventions.length} نتيجة
                </span>
              </div>
            </div>
          </CardHeader>
