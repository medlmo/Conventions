...
import { 
  ...
  insertAdministrativeEventSchema
} from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Administrative events routes
  app.get("/api/conventions/:conventionId/administrative-events", requireAuth, async (req, res) => {
    try {
      const { conventionId } = req.params;
  ...
  app.post("/api/conventions/:conventionId/administrative-events", requireAuth, requireRole([UserRole.ADMIN, UserRole.EDITOR]), async (req, res) => {
    try {
      const { conventionId } = req.params;
  ...
  app.put("/api/administrative-events/:id", requireAuth, requireRole([UserRole.ADMIN, UserRole.EDITOR]), async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.delete("/api/administrative-events/:id", requireAuth, requireRole([UserRole.ADMIN, UserRole.EDITOR]), async (req, res) => {
    try {
      const { id } = req.params;
